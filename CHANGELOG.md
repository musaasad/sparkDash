...
### Added
- **TabbyAPI READ-ONLY log-tail metrics (Qwen on DGX Spark 3)** — the TabbyAPI fork exposes NO HTTP metrics endpoint (`/metrics`, `/stats`, `/server_info` all 404) and Seq is down, so the Qwen pane read READY / "no active traffic" / all "—" while TabbyAPI was actively serving with rich per-request metrics in its own log. A new READ-ONLY collector (`server/collectors/TabbyLogProbe.js`) tails the newest `*.log` (by mtime) in a per-Spark `tabbyLogDir` over read-only SSH (`ls -t` + `tail -n 200` only — never write/rotate/delete/restart), caches on a 4s interval so a ~1MB file is not re-read every poll, and parses the exact COMPLETION/START line grammar (thousands commas, U+00B7 separators, OPTIONAL draft segment). A START id with no COMPLETION is in-flight (`active`). Real parsed numbers merge into the tabbyapi deployment telemetry with provenance `TabbyAPI log (<file>)` + `lastRequestAtMs`; `slotsTotal` comes from the only non-log metric TabbyAPI exposes (`/props` total_slots). State is honest: BUSY when in-flight, IDLE on recent completions with nothing in flight, READY/IDLE (never "no active traffic") — historical last-request perf older than 5 min with no in-flight marks STALE. GOLDEN RULE kept: these are REAL TabbyAPI-log numbers, labelled as last-request + window avg/peak (never a fabricated live gauge); missing/failed tail/unparseable line => null => "—", never 0; `tabbyLogDir` absent => honest "no log configured". No lifecycle mutation, no secrets, credential stays in the encrypted store. Preserves vllm/sglang/ds4/q27/exl3.
- **Intelligent Add-Model wizard: explicit TOPOLOGY + ROLE steps, capability-driven validation** — the guided wizard flow is now MODEL → RECIPE → RUNTIME → COMPUTE → TOPOLOGY → ROLE/OPTIONS → VALIDATE → REVIEW → SAVE. TOPOLOGY is a first-class step with all five explicit strategies (SINGLE/TP/PP/DP/EP) and free integer degrees (data-driven, never a fixed 1/2/4 enum); strategy tiers render from the runtime's DECLARED provider capability data (surfaced as `topology` on `/api/runtimes` from `registry.topologyDescriptor`) — supported strategies read calm, unknown strategies are **shown, never hidden**, and read NEEDS CONFIRMATION, unsupported ones read INVALID. A live capability + structural evaluation (`topologyCapability.evaluateTopology`, mirroring `validateTopology`) yields the DISTINCT VALID / INVALID(reason, e.g. TP4 on 2 nodes) / NEEDS-CONFIRMATION(provider capability unknown) result — UNKNOWN is never presented as VALID. ROLE is its own pure-config step over `DEPLOYMENT_ROLES` (primary|worker|specialist|reviewer|experimental|none), default none/null (never auto-assigned), written on save and changeable later via `PATCH /api/deployments/:id {role}` without recreating the model; role logic names no model. `POST /api/deployments` now accepts the optional `role`, with a `patchDeploymentRole` client helper. The VALIDATE step renders an operator-readable report (MODEL/RUNTIME/COMPUTE/TOPOLOGY/WEIGHTS/SECRETS/ROLE/LIFECYCLE) with ✓ valid, ✗ invalid(reason) and ? UNKNOWN/NOT-VERIFIED — never a false ✓ when unverifiable; LIFECYCLE reads CONFIG ONLY / DRY RUN. Discovery-first entry now clearly offers the three paths (DISCOVER RUNNING MODEL / START FROM TEMPLATE / CREATE FROM SCRATCH) with data-driven vLLM/OpenAI-compatible, TabbyAPI/EXL3, SGLang and External/observed templates. Provider topology capability moved to declarative DATA on each provider (`topology` map; tabbyapi single-only unsupported for tp/pp/dp/ep), so TP-on-tabbyapi validates INVALID while external/unknown stays needs-confirmation. SAFETY unchanged: config-only, dry-run, secret refs only, no lifecycle and no remote action.
- **Flagship cockpit redesign — model-centric cluster × per-machine telemetry** — the Overview is rebuilt to read the whole lab in 2–3 seconds while keeping every machine's telemetry one glance away. A strong compact lab header keeps the ASAD'S AI LAB identity placeholder, a single NOMINAL/ATTENTION/DEGRADED verdict and a fully data-driven briefing (`N/M COMPUTE ONLINE · K DEPLOYMENTS ACTIVE · PRIMARY: <name> · FABRIC: <state> · critical/warnings`); the instrument cluster now emphasizes the **CONFIG primary** first — greater weight via position/size/typography, never glow — followed by other active deployments and the LAB FABRIC panel. Each instrument separates three channels: FULL role label (`PRIMARY/WORKER/SPECIALIST/REVIEWER/EXPERIMENTAL/NONE`, config-first), state pill, and the tok/s gauge (centre value + unit + state word, avg/peak/trend sparkline, dynamic observed-range scaling with a value needle). Secondaries are chosen from real data — latency/queue/KV first, then subordinate GPU/CPU temp, unified memory, util, power — with `—` for anything absent. A new scalable `NodeTelemetryStrip` gives a slim row per node (1..N) with online state, measured temps, memory used/total/% + free, util, power, deploy membership and role — no three giant cards, no extra polling, no fabricated thermal thresholds (classification only from provider throttle). Unknown ≠ failed, idle ≠ offline, role ≠ state ≠ performance; unified-memory terminology honoured and power limit shown only when known.
- **AI Lab Cockpit (Overview as a live instrument cluster)** — Overview is now a premium digital instrument cluster that reads the whole lab at a glance, built inside the existing token system (no gradients/glass/glow/RGB, zero hardcoded hex). L1 glanceable header (AI LAB wordmark + a single NOMINAL/ATTENTION/DEGRADED verdict + data-driven summary); one LARGE instrument per active deployment (primary-first) with a `ThroughputGauge` — a restrained SVG radial arc scaled to a **rolling observed range (never a fake x/100)**, large central value + TOK/S, trend + sparkline; PRIMARY/WORKER role (config-driven `role`, heuristic fallback); placement via `TopologySummary`; runtime/context/uptime; a state pill from `deriveRuntimeState` (READY/IDLE/SERVING/BUSY/DEGRADED/OFFLINE/UNKNOWN); and secondary instruments chosen from each runtime's **declared** metrics (KV/QUEUE/RUNNING for vLLM, MTP/CACHE/TTFT for TabbyAPI) — null-safe, never fabricated. An honest **LAB FABRIC** panel shows physical nodes + link speed + placement + health via `deriveFabric`/`fabricLayout`, drawing edges **only where discovered** (else a clear "edges not discovered" caption), kept SEPARATE from deployment topology and adaptive to 1..N nodes. `useTelemetryHistory` ring buffer feeds gauge scaling/sparkline/trend/recency. Responsive (verified 1920/1440/720). A healthy auth-gated/external runtime reads calm READY (never a scary DEGRADED or false 0); unreadable external telemetry shows "throughput unknown / metrics require key".
- **Per-deployment telemetry + operational state** — `DeploymentView` carries a normalized `telemetry` object from the primary-node llm probe (generationTps, prefillTps, ttftSeconds, requestsRunning/Waiting, kvCacheUsage, prefixCacheHitRate, mtpAcceptanceRate, contextLength, gpuMemoryUtilization, slots, backend, modelId), multi-node generationTps summed; `deriveRuntimeState` classifies offline/degraded/busy/serving/idle/ready/unknown. Each `RuntimeProvider` declares the normalized metrics it meaningfully exposes (surfaced via `/api/runtimes` `metrics[]`) so the cockpit shows only real secondary instruments per runtime.
- **Future-ready topology + physical fabric** — `normalizeTopology` accepts explicit `tp/pp/dp/ep/coordinator/workers` (backward-compatible with legacy `mode`+`parallelism` and the `tp2` slug). **Parallelism is NEVER inferred from node count** — unknown degrees stay null and set an `unknown` flag when >1 node; TP2+PP2 and SINGLE/TP2/PP3/TP4 are representable. A new `fabricModel` derives the PHYSICAL fabric separately from deployment topology (links only where a shared CX7 subnet / configured fabric is discovered); `fabricLayout` lays out 1..N nodes; `TopologySummary` renders "TP2 · DGX 1+2" / "Single · DGX 3" / "2 nodes · topology unknown".
- **Intelligent model setup — Discover → provenance → template → guided wizard** — "Discover running model" is a first-class path (Models toolbar + template picker). `discoverEndpoint` (read-only GET /v1/models via the existing probe stack) detects runtime/served-model/api-protocol/context/quantization with **per-field provenance** (detected/probed/user/unknown) and a **suggested template** (overridable, with confidence); an **opt-in tiny** `probeCapabilities` (max_tokens 1, never a benchmark) checks text/streaming/vision/tools/reasoning. The SAME 8-step `ModelWizard` takes a `seed` and prefills Model/Recipe/Runtime/Compute/Options with inline provenance badges (every value editable, UNKNOWN flagged for confirm); the Compute step separates PHYSICAL placement from DEPLOYMENT topology (explicit TP/PP/DP/EP, never inferred from node count); Review shows a provenance-annotated config-only summary with a "CONFIG ONLY — no lifecycle action" note; a discovered external endpoint stays **External / observed** (discovery ≠ ownership). Family/weight-path are never fabricated for external endpoints (blank + UNKNOWN until the operator resolves). Bounded discovery: known nodes + configured/recipe ports + an explicit typed host:port — never a blind LAN sweep.
- **Config-first v2 domain core (WS-1)** — Model → Recipe → Deployment → Runtime → Compute are now cleanly separated, versioned (`schemaVersion: 2`) and configuration-driven, so onboarding a model/recipe is DATA, not frontend code. MODEL holds weights identity (`weightPaths` variants); RECIPE is declarative and reusable (engine/serving/launch/topology/healthProbe/discovery/logSource/lifecycleCommands) with **no** node bindings, weight paths or raw secrets; DEPLOYMENT binds model+recipe+nodes+desiredState; RUNTIME is the observed per-deployment view. Idempotent v1→v2 migration with `.v1.bak` backups; recipe lifecycle (draft→validated→proven→deprecated→archived) with legal-transition guards; duplicate deep-copies a proven recipe as a draft without touching the original; env secrets are `secretRef`-only (values in the encrypted store, never in config/API). The two live environments are now committed seed DATA (`server/seeds/*.json`), not code — proof: a hypothetical model + recipe onboard via the API with zero frontend edits.
- **LangChain design system, light + dark (WS-2)** — the user-selected LangChain/Mobbin reference (`docs/DESIGN_LANGCHAIN.md`, Kimi-extracted) is now the design north star in BOTH themes as hand-tuned variants (never an inversion); SparkDash keeps its amber accent, adopting LangChain's structure/hierarchy/density only. Four hand-specified `[data-theme]` token blocks (dark/light/white/oled) + spacing/radii/height tokens; gradient/glass/glow/left-border-active/pill-counts retired (locked design bar). 230px flat rail nav with grouped hairlines, full-width amber selected tint, muted counts, pinned Settings + workspace card; Breadcrumb-over-H1 + PageHeader grammar. Every content surface restyled to hairline tables, dot+word status pills, text-only underline tabs, one-primary-per-view, 440px verb-named modals, spec empty/loading/error/offline states — fully token-driven (zero hardcoded hex in components).
- **Guided onboarding + recipe management (WS-4)** — an 8-step Add-Model wizard (Model→Recipe→Runtime→Compute→Options→Validate→Review→Save) with progressive disclosure, smart defaults, inline validation and a config-only Save that lands on the new model; first-class recipe cards with lifecycle badges, structured v2 editor, Validate, Duplicate-as-draft and Deprecate/Archive; model-detail IA surfaces Overview | Recipes | Deployments | Live Console | Benchmarks. Safe lifecycle semantics are separated and surfaced via modals: STOP ≠ delete recipe, REMOVE binding ≠ delete model, ARCHIVE never deletes weights, delete-weights stays out-of-scope.
- **Provider abstraction, read-only discovery + config-only adoption (WS-3)** — a runtime-keyed provider registry (`server/domain/providers/`) encapsulates every runtime-specific behaviour that used to be a scattered `switch`: `detect`, `healthClassify` (WS-1 401/403 = up semantics), read-only served-model path, dry-run `renderLaunchCommand`, and runtime log/telemetry shaping (loguru/TabbyAPI moves behind TabbyApiProvider, collector goes generic). `validate.js`/`controlPlane.js`/`LiveConsole.js` now ask the provider registry, not the runtime string. A read-only `DiscoveryService` (`config/discovered.json`, gitignored runtime state) GET-probes each node's inference ports via `/v1/models` + read-only `pgrep`, correlates against existing deployments, and surfaces **uncovered serving endpoints** — offline-safe, no fabricated entries. `GET /api/discovery` + `POST /api/discovery/:id/adopt` (associate or create) write config only and create a deployment bound to the node with `desiredState: unknown` and provenance; the discovered process is never started, stopped, signalled or reconfigured. Models shows a "Discovered runtimes (N)" band with a Review & Adopt modal (associate-existing / create-new / review step); Node detail scopes it per node.
- **Desired-vs-observed runtime state model** — deployments now carry `desired` (configured intent) and `observed` (probe classification: HTTP 2xx → running, **401/403 → auth-gated (the process IS up)**, refused/DNS → not-detected, 5xx/timeout → unhealthy) plus read-only SSH `pgrep` corroboration. Display vocabulary: `Running` / `Running external` / `Expected · not detected` (warning) / `Degraded` / `Stopped`. Fixes the trust-destroying case where Qwen — serving through auth-gated TabbyAPI launched outside SparkDash — was shown as "Stopped"; it now reads **Running · external**. No process control was added: lifecycle stays dry-run, corroboration is strictly read-only.
- **Live Console operator experience** — field-aware query toolbar (`reqId:`, `model:`, `node:` + free text), time-window chips, per-deployment source tabs, explicit Live toggle; Following/Paused stream state with scroll-up auto-pause + sticky resume and disconnect banner naming the node; 32px parsed rows with copy-on-click mono reqId and fixed-order metric chips (prompt/gen tokens, cached %, prefill, TTFT, decode, MTP accepted/attempted, tool calls, duration — only fields actually parsed, never fabricated); Parsed↔Raw drawer with per-field copy and "View in Activity" preserving the reqId (deep link now round-trips through the router); events-per-minute histogram scrubber with error overlay; severity facets with live counts; copy-visible + JSONL export; p90+ outlier tinting.
- **Commercial polish pass (design brief: Kimi × Mobbin research, `docs/DESIGN_BRIEF.md`)** — counted status tabs + toolbars on Models/Fleet/Activity; deployment row grammar (pill → mono id → age+provenance → node cluster chips → port → View logs); read-only external connect panel (copyable endpoint, runtime-aware "manage via vLLM/TabbyAPI" copy, disabled controls with dry-run notice); model catalog family grouping; Fleet health rail with working attention filter; node detail underline tabs + big-number panels; Settings subsystem rail + radio cards + type-the-name Danger Zone; Benchmarks p50/p90 tiles, signed comparison deltas, proportional token bars.
- **Accessibility** — roving-tabindex + arrow-key navigation on all tablists, Space activation and proper roles on interactive table rows.
- **Domain foundation: roles, provider topology capability, capability-aware validation, fabric provenance** — `DEPLOYMENT_ROLES` widens to `primary|worker|specialist|reviewer|experimental|none` with legacy `edge`→`worker` back-compat (additive, no schemaVersion bump, idempotent normalizers, existing v2 data stays valid); a role change is a **config-only** `PATCH /api/deployments/:id {role}` (no model/recipe recreation, DeploymentService projects role). `RuntimeProvider.supportsTopology({mode,degree,nodeCount}) → supported|unsupported|unknown` (base/external default **unknown**, vllm/sglang declare tp by node count, tabbyapi single-only) exposed via `registry.supportsTopology`. New `validateTopology()` combines structural rules (degree ≤ nodeCount, degree product ≤ nodeCount, single ⇒ 1 node) with the provider declaration → `valid|invalid|needs-confirmation` (structural violation or `unsupported` ⇒ invalid; `unknown` on a fine multi-degree ⇒ needs-confirmation, never silently valid; node count NEVER becomes a degree) and is wired into `recipeValidate` as an error/warning. `fabricLinks` becomes a READ-ONLY `SparkConfig`/`SparkSnapshot` passthrough (like `cx7Ip`); `fabricModel` emits links with explicit `provenance: configured|discovered` (configured from config, discovered from shared CX7 subnet) and keeps `wiringDiscovered=false` when neither — no fabricated triangle. Committed `server/seeds/compute.json` seeds an empty fleet with 3 DGX nodes + the configured fabric triangle (the gitignored live `config/sparks.json` is never hand-edited).

### Fixed
- Fleet header node count now reflects the whole fleet (3 nodes · 1 worker) instead of the main-table subset; Fleet topology uses friendly model names.
- Live Console distinguishes "no log directory configured" (neutral, no false Reconnect promise) from an actual stream disconnect (amber + Reconnect).
- Console reqId extraction is anchored (`reqId:` field or `#NNN` token) — arbitrary digit runs in log lines are no longer mistaken for request ids.
- Fleet "attention" rail bucket matches only nodes with real conditions; count and filter share one source.
- Control-plane tables scroll horizontally below ~900px instead of clipping columns.

### Tests
- Wizard role step (6 roles, config-only, null default, no model-name logic), topology capability matrix (vllm TP2/2 valid, TP4/2 invalid, TP3/3 unknown ⇒ needs-confirmation, tabbyapi TP ⇒ invalid, 3 nodes no degree ⇒ unknown), validate report honesty (? for unverifiable, never ✓), discovery three-path entry, and server descriptor/validation wiring — frontend 365 → **397**, server 460 → **463**.
- Frontend 78 → **182**, server 351 → **367**; state-derivation matrix, probe classification, console query pipeline, follow/pause reducer, JSONL export shape, counted-tabs, dirty-state gating, type-the-name confirmation all covered.
- Cockpit redesign: frontend 311 → **339** — full-role matrix (incl. NONE/edge), config-primary-follows-offline, node-telemetry honesty (`—` not 0, power-limit-absent, unified vs RAM, throttle-only classification), node strip scalability/empty/offline, gauge needle+state-separation, header briefing + primary emphasis.

#### Earlier in this cycle — control-plane foundation

### Added
- **AI Lab Control Plane** — sparkDash grows from monitoring into an observe → understand → operate surface for the whole lab. New fixed section navigation **Overview · Models · Fleet · Activity · Benchmarks · Settings** replaces the tab-first shell (the old Spark tab strip survives as the node sub-nav; legacy `/spark/:id` URLs keep resolving). Server-side **Model → Deployment Recipe → Runtime → Nodes** registry (`config/models.json`, `config/recipes.json`, gitignored): recipes carry runtime, model path, workdir, topology (single/TP2/TP3), nodes, API port, context, env vars (secret values never leave the server), CPU affinity, launcher and notes. Archive never deletes weights; a model with recipes cannot be hard-deleted.
- **Live Console** — read-only tail of the model server's own log stream over SSH (`tail -F` on the recipe's log dir). **Telemetry View** parses TabbyAPI/loguru inference lines into one row per request: prompt/gen tokens, cached %, new prompt tokens, prefill tok/s, TTFT, decode tok/s, MTP draft accepted/attempted + acceptance %, tool calls, duration. **Raw Console** follows the stream with pause/follow, search, copy, and level-colored lines. Nothing about the model process is touched — the console is strictly read-only.
- **Deployment lifecycle (dry-run)** — Start / Stop / Restart simulate the state machine (`available → starting → loading → running → stopping → stopped`, `error`), persist a checkpoint, and write audit entries (`/api/audit`). No process is launched or stopped this phase; externally managed deployments (the seeded Qwen 3.8 on dgx-3) render controls **disabled with an explicit reason**. Loopback dry-run requires `SPARKDASH_ALLOW_DRYRUN_LOOPBACK=1`; remote binds need `SPARKDASH_TOKEN` and still fail closed.
- **Qwen 3.8 example deployment seeded** — first real deployment recorded as generic architecture metadata (dgx-3, TabbyAPI+EXL3, port 8889, ctx 262144, affinity `5-9,15-19`, EXL3 env vars, taskset launcher), marked externally managed / observe-only.
- **Activity feed** — real events only: node online/offline transitions, lifecycle operations, benchmark/showcase completions (tagged with the serving recipe when resolvable), alerts. Benchmarks section aggregates fleet bench history; runs are only compared when kind, recipe and workload shape match.
- **Recipe wizard** — 3 steps (Identity → Runtime & Compute → Review) with collapsed Advanced (health path, affinity, log dir, launcher, env vars incl. secret flag), inline validation mirroring the server rules; clone and archive supported.
- **New UI primitives** — `DataTable` (sortable, aria-sort, tabular numerals), `StatusPill`/`StatusDot` (transitional states pulse, never borrow success), `TimeSeriesChart` (fixed right-anchored window, hover tooltip, range picker), form kit (`Field`/`FormSection`/`AdvancedDisclosure`/`FormFooter`), `EmptyState`, status color tokens (`--color-status-*`, `--color-info`) across all four themes; pre-paint theme script kills the light-flash on load.
